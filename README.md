...
<details>
  ...
  📑 따라서 한 방의 클라이언트들이 각자 게임 로딩이 다 완료되면 spring에 request를 보냈고 spring에서는 request가 올때마다 count를 세서 count가 현재 한 방의 인원들의 숫자와 같아지면 게임을 시작시키는 방식으로 하려했습니다.
  📑 여기서 문득 떠오른게 만약 로딩중에 누군가가 나가면 무한대기현상이 발생할 수 있다고 생각했습니다. 왜냐하면 나간 사람은 영원히 spring에 로딩이 다 되었다는 request를 보내지 않을 것이기 때문입니다.
  📑 구체적으로 분석해보자면 당시 노드 socket에서 유저 disconnect가 발생하면 스프링에서 1. 방장이 나간 경우: 새로운 방장 userId만 넘겨준다.	2. 일반인이 나간 경우: null값을 넘겨준다.
  📑 스프링에서 게임 로딩 체크할때는 1. false만 계속 보내주다가 2. 마지막 인원까지 로딩이 완료된다면 true값을 보내줍니다.
  📑 문제는 동시에 발생하는 경우입니다. 즉, 게임 로딩중에 누군가가 disconnect가 된다면 최악의 경우 새로운 방장 userId와 게임 무한 대기 현상을 방지하기 위해 마지막 인원까지 로딩이 완료되었다는 true값도 보내줘야합니다. (방장이 나간 경우, 나간 사람 빼고 전부다 로딩 된 경우)
  📑 따라서 생각해낸 것이 누군가가 나갔을때 넘겨주는 값의 형식과 게임 로딩체크할때 넘겨주는 값의 형식은 같아야한다는 것이였습니다. 즉, 누군가가 나간다면 userID만 넘겨주는 것이 아니라 userId와 true, false값을 같이 보내주고, 반대로 게임 로딩중에도 true, false뿐만 아니라 userId까지 보내주는 것입니다.
  📑 이런 방식으로 한다면 게임 로딩중에 누군가가 나간다면 프론트쪽에서 true 또는 false값도 받을 수 있게 되므로 무한대기현상을 해결할 수 있다고 생각했습니다.
  📑 즉, 경우의 수를 따져보면 1. 게임로딩 X, 누군가가 나감 ----> 	1. 방장이 나간경우: {"userId" : "새로운 ID", "check": null}		2. 일반인이 나간경우: {"userId": null, "check": null}
  ...
  📑 3. 일반적인 게임 로딩 --> 1. {"userId": null, "check": null} ........ 2. 제일 마지막 로딩: {"userId": null, "check":"true"}이 되는 것이다. 
  📑 위와 같은 방식으로 해결이 가능할 것이라 생각했었습니다. 
  📑 하지만 disconnect가 발생하면 한명의 Client가 spring에 request를 보내는 것이 아니고 방 전체 인원들이 Spring으로 request를 보내기 때문에 방장이 나간 경우 나간 유저를 삭제하고 새로운 방장을 만들어주는 로직이 한번만 이뤄지는 것이 아니라 여러번 발생하게 되면서 문제가 발생합니다(이미 삭제된 유저를 삭제하려고 한다던지 같은 문제들).
  📑 따라서 한번만 요청하는 로직을 만들기 위해서는 위에서 생각한 방식말고 node에서 socket disconnect시에 한번만 처리하는 것으로 해결되었습니다. 
  ```
